...
export type FuelPriceSyncResult = FuelPriceSnapshot & {
    syncedCount: number;
};

export const SOURCE_URL = 'https://www.eppo.go.th/wp-json/oil-api/v1/oil-prices';
...
export function toFuelPriceDbRows(rows: FuelPriceRow[], effectiveDate: string): FuelPriceDbRow[] {
    return rows.map((row) => ({
        brand: row.brand,
...
export async function fetchEppoFuelPrices(): Promise<FuelPriceSnapshot> {
...
export async function syncFuelPrices(repository: FuelPricesRepository): Promise<FuelPriceSyncResult> {
    const snapshot = await fetchEppoFuelPrices();

    const { error } = await repository
        .from('fuel_prices')
        .upsert(toFuelPriceDbRows(snapshot.rows, snapshot.effectiveDate), {
