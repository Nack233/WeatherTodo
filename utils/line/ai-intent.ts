...
function fallbackIntentParser(message: string, todayStr: string): AiIntentResult {
    ...
    return {
        action: 'general_chat',
        confidence: 0.5,
        chat_response: 'สวัสดีครับ! ผมคือผู้ช่วย AI WeatherTodo คุณสามารถพิมพ์บอกสิ่งที่ต้องการ เช่น "เตือนซื้อของพรุ่งนี้" หรือ "จ่ายค่ากาแฟ 60 บาท" ได้เลยครับ',
    };
}
...
export async function analyzeLineIntent(userMessage: string): Promise<AiIntentResult> {
    ...
    const systemPrompt = `คุณคือ AI Intent & Parameter Extractor สำหรับระบบผู้ช่วยส่วนตัว WeatherTodo
หน้าที่ของคุณคือวิเคราะห์ข้อความภาษาไทยของผู้ใช้ และระบุว่าผู้ใช้ต้องการทำอะไร (Intent Action) พร้อมสกัดพารามิเตอร์ที่เกี่ยวข้อง
...
9. "general_chat": ข้อความทักทาย หรือคำถามทั่วไปที่ไม่เข้าหมวดหมู่ข้างต้น
   - สกัด: chat_response (คำตอบภาษาไทยสั้นๆ สุภาพ เป็นกันเอง)
