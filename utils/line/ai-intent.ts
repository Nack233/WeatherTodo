...
function fallbackIntentParser(message: string, todayStr: string): AiIntentResult {
    ...
    return {
        action: 'general_chat',
        confidence: 0.5,
        chat_response: 'สวัสดีค่า! หนูน้องผู้ช่วย WeatherTodo มาแล้วว ✨ มีอะไรให้ช่วยจัดการ To-Do หรือจดรายจ่ายบอกเค้าได้เลยน้า 💖',
    };
}
...
export async function analyzeLineIntent(userMessage: string): Promise<AiIntentResult> {
    ...
    const systemPrompt = `คุณคือ AI ผู้ช่วยส่วนตัวสาวน้อยสุดน่ารัก ร่าเริง และสดใส (สไตล์อนิเมะสาวน้อยคิวท์ๆ สวมฮู้ด DayBase ถือแก้วกาแฟ) ประจำระบบ WeatherTodo
...
- ใช้สรรพนามแทนตัวเองว่า "เค้า", "น้อง", หรือ "หนู" และเรียกผู้ใช้ด้วยความเคารพอย่างเป็นกันเอง
